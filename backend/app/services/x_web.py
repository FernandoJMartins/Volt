...
async def _extract_tweet(article) -> dict | None:
    ...
    photo_urls: list[str] = []
    for img in await article.query_selector_all(f'{SEL["photo"]} img'):
        src = await img.get_attribute("src")
        if src and "pbs.twimg.com/media" in src and src.split("?")[0] not in photo_urls:
            photo_urls.append(src.split("?")[0])
    ...
    return {
        ...
        "media_metadata": {"photo": has_photo, "video": has_video, "images": photo_urls},
    }
